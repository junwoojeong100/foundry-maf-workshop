...
| **어디서 쓰나?** | 포털([ai.azure.com](https://ai.azure.com)) + Azure 리소스 | 내 코드 (`pip install agent-framework`) |
...
```bash
python --version
# 예: Python 3.11.5  → OK (3.10 미만이면 업그레이드 필요)
```
...
- Foundry 리소스를 만들려면 구독에 대한 **Owner** 또는 **Contributor** 역할이 필요합니다.
...
- `azure-ai-projects>=2.0.0` — Foundry SDK (Lab 02 기초 예제)
- `agent-framework` — Agent Framework 코어 (Lab 03~06)
...
> 💡 이 실습의 코드는 `DefaultAzureCredential` / `AzureCliCredential`을 사용합니다.
...
- [ ] `az --version` → 정상 출력
- [ ] `pip install -r requirements.txt` → 오류 없이 완료
- [ ] `az login` → 로그인 성공
