...
> **명령 단계 구분:** `azd ai agent init`은 로컬 프로젝트 파일과 azd 환경을 초기화할 뿐, 소스나 이미지를 빌드·푸시·배포하지 않습니다. 실제 패키징·빌드·배포는 `azd up` 또는 `azd deploy`에서 수행됩니다. `azd up`은 필요한 프로비저닝까지 포함하고, `azd deploy`는 이미 준비된 환경에 새 agent 버전을 배포합니다.
...
두 방식 모두 결과는 **Foundry hosted agent**이지만 배포 아티팩트와 빌드 경로가 다릅니다. Code deployment는 소스 ZIP을 사용하며 사용자 관리 컨테이너 이미지나 ACR이 필요하지 않고, container deployment는 Dockerfile로 이미지를 만들어 ACR에 저장합니다.
...
| 추가 요구사항 | Docker/ACR 불필요 | ACR, ACR 빌드 권한 (Docker는 로컬 빌드·검증 시에만 필요) |
...
이 문서는 **2026-07-13**에 다음 조합으로 검증했습니다.

| 구성 요소 | 검증 버전 |
|-----------|-----------|
| Azure Developer CLI (`azd`) | `1.27.0` |
| `microsoft.foundry` 확장 | `1.0.0-beta.1` |
| `azure.ai.agents` 확장 | `1.0.0-beta.5` |

> Foundry 확장은 preview/beta 단계이므로 새 버전에서 플래그나 생성 파일이 달라질 수 있습니다. 버전이 다르면 `azd ai agent init --help`와 공식 quickstart를 먼저 비교하세요.
...
Container deployment의 기본 ACR 원격 빌드(`docker.remoteBuild: true`)는 로컬 Docker 없이 수행할 수 있습니다. 아래 C-6에서 이미지를 직접 빌드하고 `/readiness`를 확인하려면 [Docker](https://docs.docker.com/get-docker/)가 필요합니다. 원격 빌드 실행 사용자에게는 대상 ACR의 **Container Registry Tasks Contributor** 역할이 필요하며, `azd`는 배포된 agent identity의 이미지 pull 역할을 자동으로 구성합니다.
...
> 이 단계는 선택입니다. Docker가 없다면 C-7로 이동해 기본 ACR 원격 빌드를 사용하세요.
...
> `AZURE_CONTAINER_REGISTRY_ENDPOINT`가 출력되지 않아도 오류가 아닙니다. 기존 프로젝트에 ACR 연결이 없다면 container 배포에 필요한 프로젝트용 ACR을 다음 `azd up`의 프로비저닝 단계에서 만들 수 있습니다.

기존 프로젝트에 ACR 연결이 하나 있으면 `azd ai agent init`이 자동 선택합니다. 조직의 공용 ACR을 사용하려면 [공식 ACR 가이드](https://learn.microsoft.com/azure/foundry/agents/how-to/deploy-hosted-agent-private-azure-container-registry)에 따라 다음 두 값을 설정합니다.
...
| Docker가 실행되지 않음 | C-6 로컬 검증을 생략하고 기본 ACR 원격 빌드를 사용하거나, Docker Desktop/Engine을 시작해 `docker version`의 Client와 Server를 확인 |
